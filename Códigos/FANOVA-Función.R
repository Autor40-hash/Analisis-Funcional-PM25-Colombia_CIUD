...
#' 
#'@export 
fanova.onefactor2 = function(object, group, nboot=100,
  ...
  if (plot){  
    ...
    legend("topright",legend = c("Total", levels(group)),lwd = 2,col = 1:(nk+1),bty = "n",cex = 1)
...
bootanovafunc<-function(indi,xm,cov,nboot,plot=FALSE,rangedat=NULL){
  ...
  if (plot) {
    ...
    legend("topright",legend = c("Total", levels(indi), "Resamples"),lwd = 2, col = c(1,2:(nk+1),"grey"),bty = "n",cex = 1)  
    #legend("bottom",legend = c("Total", levels(indi), "Resamples"),lwd = 2,col = c(1,2:(nk+1),"grey"),bty = "n",horiz = TRUE,cex = 1.1,inset = -0.25,xpd = TRUE)
  }
  wm<-numeric(nboot)
  xm3<-NULL
  for(inb in 1:nboot){
...
# La función original generaba los gráficos de manera automática; 
# sin embargo, las leyendas y algunos elementos visuales no presentaban 
# una adecuada legibilidad. Por esta razón, se realizó una modificación de 
# la función original (código anterior) con el fin de mejorar la 
# visualización de los resultados y garantizar una correcta interpretación 
# de la información presentada.

par(mfrow = c(1,3),mar = c(5,5,5,2),oma = c(0,0,2,0),cex.main = 1.2,cex.lab = 1.1,cex.axis = 1)
set.seed(123)
result2 = fanova.onefactor2(fdata,zona,nboot = 50,plot = TRUE)
